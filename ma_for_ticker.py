...
def get_graph_by_ticker(ticker: str):
    get = client.get_market_search_by_ticker(ticker)
    get = [(get.payload.instruments[0].figi, get.payload.instruments[0].name)]
    fig = get_figure(get)
    ma = get_figi_data(get[0])
    for period in ma_period:
        fig.add_scatter(name="EMoving average " + str(period), y=ma["c"].ewm(span=period,adjust=False).mean(), x=ma["time"])
    for period in ma_period:
        fig.add_scatter(name="Moving average " + str(period), y=ma["c"].rolling(window=period).mean(), x=ma["time"])
    fig.show()
...
def get_buy_points(figi:str) -> List:
    df = get_figi_data(figi)
...
def get_sell_points(figi:str) -> List:
    df = get_figi_data(figi)
    data = [df["c"].rolling(window = 10).mean(),df["c"].rolling(window = 60).mean(),df["time"]]
    result = []
    for i in range(len(data[0])):
...
def tester(figi: str)-> int:
    df = get_figi_data(figi)
    print (df)
    sell_points = get_sell_points(figi)
    buy_points = get_buy_points(figi)
    result =[]
    s=0
    b=0
